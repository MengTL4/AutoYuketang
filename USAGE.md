...
```bash
python -m pip install -U requests pillow fonttools pytest
```
...
- 如果课程里存在未完成的讨论学习点或练习学习点，`api_key` 必须填写。
- 如果你只想跑视频学习点，可在 `yktMain.py` 里临时注释讨论/练习学习点循环。
...
- `api_key` 未填、无效，或不是可用的 DeepSeek Key
- 模型接口不可达
...
- `api_key` 未填、无效，或不是可用的 DeepSeek Key
- 题目字体下载失败，导致题干无法解码
- 大模型返回了不合法的答案格式，代码会跳过该题并记录日志

如果想先验证其他能力，可以先临时跳过练习学习点循环。
